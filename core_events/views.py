...
from core_events.models import Calendar, Event, EventMedia
from django.contrib.auth.decorators import login_required
from core_events.forms import AddEventForm, EventMediaFormSet
...
def events_list(request, calendar_id):
    ...
    return render(request, 'events/events_list.html', {'events_list': events, 'calendar': calendar})
...
def add_event(request, calendar_id):
    ...
    if request.method == 'POST':
        ...
        form = AddEventForm(initial={'date_of_start': initial_date})
...
def edit_event(request, calendar_id, event_id):
    ...
    if request.method == 'POST':
        ...
        event_media_formset = EventMediaFormSet(request.POST, request.FILES, instance=event)
        if form.is_valid() and event_media_formset.is_valid():
            form.save()
            event_media_formset.save()
...
def delete_event(request, event_id, calendar_id):
    ...
    return render(request, 'events/delete_event.html', {'profile': profile, 'event': event, 'calendar': calendar})
...
def calendar_events(request, calendar_id):
    ...
    cal = EventCalendar(year, month, calendar)
...
def add_calendar(request):
    ...
    if request.method == 'POST':
        ...
        Calendar.objects.create(name=name)
...
def calendars_list(request):
    ...
    return render(request, 'events/calendars_list.html', {'calendars_list': calendars})
