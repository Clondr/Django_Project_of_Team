...
from core_forum.models import ForumPost, ForumComment
from core_forum.forms import ForumPostForm, AddCommentForumForm
...
def forum_comments_list(request, post_id):
    ...
    return render(request, 'forum/forum_comments/forum_comments_list.html', {'forum_comments_list': comments, 'post': post})
...
def edit_forum_comment(request, forum_comment_id, post_id):
    ...
    if request.method == 'POST':
        form = AddCommentForumForm(request.POST, request.FILES, instance=forum_comment)
        if form.is_valid():
            form.save()
